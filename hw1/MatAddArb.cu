...
__global__ void matrixAdd(float *Md, float *Nd, float *Pd, int width, int height)
{
    ...
    if(id_y < width && id_x < height){
        for(int i = 0; i< width; ++i){
            Pvalue += Md[id_y*width+i] + Nd[i*height+id_x];
        }
    }
    Pd[id_y*width + id_x] = Pvalue;
    // print input submatrices
    // for(int i = 0; i < 5; ++i){
    //     printf("Row/Col = (%f)\n",
    //         Pd[i]);
    // }
...
int main( int argc, char* argv[] )
{
    ...
    // Execute the kernel:
    matrixAdd<<<dimGrid, dimBlock>>>(Md, Nd, Pd, width, height);
