...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
  ...
  int gridSizeX = (int) ceil((float)P.width/blockSize);
  int gridSizeY = (int) ceil((float)P.height/blockSize);
  dim3 dimBlock(blockSize, blockSize); // number of thread blocks
  dim3 dimGrid(gridSizeX, gridSizeY); // number of thread per block
