...
float computeOnDevice(float* h_data, int array_mem_size, int kernel);
...
void runTest( int argc, char** argv) 
{
    ...
    {      
        ...
        case 1:
            ...
            {
                //get which kernel need to execute
                kernel = atoi(argv[1]);
    ...
    float result = computeOnDevice(h_data, num_elements, kernel);
    // **===-----------------------------------------------------------===**
...
float computeOnDevice(float* h_data, int num_elements, int kernel)
{
    float* d_data;
    //float* d_output;
    ...
    printf("(length of vector, kernel) --> (%d, %d) \n", num_elements, kernel);
    if(kernel == 0) reduction<<<gridSize, blockSize>>>(d_data, num_elements);
    if(kernel == 1) reductionLarge<<<gridSize, blockSize>>>(d_data, num_elements);
